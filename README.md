...
업종에서 멈추지 않고 실제 상장사 이름까지 적되, `data/krx_names.csv`(corp_master 에서 `export_krx_names.py` 로
추출한 현재 상장사 2,879개, 월 1회 갱신)에 있는 이름만 쓴다 — 클라우드 루틴이 비상장사나 없는 이름을 지어내는 것을 막는 장치.
